...
/// Kitty class for the Neovim editor window of the nix-config layout.
///
/// Neovim runs inside Kitty, and Kitty DOES support `--class`. That is the one
/// place the Neovim layouts are simpler than the Zed ones: their placement
/// rules match on class alone, so unlike `ZED_CLASS_PATTERN` above they carry
/// no dependency on the project folder name being the window title.
const NIXCFG_NVIM_CLASS: &str = "nixcfg-nvim";

/// Kitty class for the nix-config layout's terminals.
///
/// Shared by both editors on purpose: the Zed and Neovim nix-config layouts are
/// alternatives occupying the same workspace, never both at once (see the guard
/// in `nix_config_layout`), so one terminal class covers them.
const NIXCFG_TERM_CLASS: &str = "nixcfg-term";

/// Neovim keybind reference shown in the layout's top-right pane, relative to
/// $HOME. Generated by `home/modules/neovim.nix` from the same attrset that
/// defines the keymaps, so it cannot drift from the bindings it documents.
const NVIM_KEYBINDS_REL: &str = ".config/nvim/KEYBINDS.md";

/// Which editor a layout is built around.
...
fn run() -> Result<()> {
    match parse_args(env::args().skip(1).collect())? {
        ...
        Mode::NixConfig { editor } => nix_config_layout(editor),
        Mode::New { path, editor } => new_dev_space(path.as_deref(), editor),
    }
}
...
enum Mode {
    Help,
    NixConfig { editor: Editor },
    New { path: Option<String>, editor: Editor },
}

fn parse_args(args: Vec<String>) -> Result<Mode> {
    let mut new_space = false;
    let mut path: Option<String> = None;
    let mut editor = Editor::Zed;

    for arg in args {
        match arg.as_str() {
            "-h" | "--help" => return Ok(Mode::Help),
            "--new" => new_space = true,
            "--nvim" => editor = Editor::Neovim,
...
fn print_help() {
    ...
    println!("    dev-layout --nvim          as above, but Neovim in Kitty instead of Zed");
    println!("                               (combines with --new)");
    println!("    dev-layout --help          show this help");
    println!();
    println!("Re-running a nix-config layout focuses the existing one rather than");
    println!("stacking a second set of windows on workspace {NIX_CONFIG_WS}.");
    println!();
...
fn nix_config_layout(editor: Editor) -> Result<()> {
    // Both the Zed and the Neovim nix-config layouts live on workspace
    // NIX_CONFIG_WS, so without this a second press of either keybind stacks a
    // fresh editor plus two terminals on top of whatever is already there.
    // Neither mode had the check before, which is why the Zed keybind used to
    // be additive rather than idempotent.
    if workspace_is_occupied(NIX_CONFIG_WS)? {
        println!(
            "{} {}{}",
            "nix-config layout already open on workspace".green(),
            NIX_CONFIG_WS.to_string().cyan(),
            "; focusing it.".green()
        );
    ...
    if !repo_path.is_dir() {
        warn(&format!(
            "{repo} is not a directory; {} may open an empty window.",
            editor.name()
        ));
    }
    ...
    // Zed only. The Neovim window carries an explicit --class, so its placement
    // does not depend on the folder name at all and this warning would be a lie.
    if editor == Editor::Zed && repo_path.file_name() != Some(OsStr::new(NIX_CONFIG_FOLDER)) {
    ...
    // Spawn the editor. No per-launch rules needed: the static window rules pin
    // the Zed window (class + title), the Neovim window (class NIXCFG_NVIM_CLASS)
    // and the NIXCFG_TERM_CLASS terminals alike.
    spawn_editor(editor, repo_path, NIXCFG_NVIM_CLASS)?;
    thread::sleep(Duration::from_secs(1));

    // Two terminals, tagged so the workspace 2 rule can match them.
    spawn_layout_terminals(editor, NIXCFG_TERM_CLASS, repo_path)?;
...
// ── Shared window spawning ────────────────────────────────────────────

/// Spawn a layout's editor window.
///
/// Zed takes the project as an argument. Neovim is launched inside a Kitty
/// window whose `--class` is what the placement rule matches, with the project
/// as the working directory so telescope, `:e` and toggleterm all start there.
fn spawn_editor(editor: Editor, project: &Path, nvim_class: &str) -> Result<()> {
    let mut command = match editor {
        Editor::Zed => {
            let mut c = Command::new("zeditor");
            c.arg("-n").arg(project);
            c
        }
        Editor::Neovim => {
            let mut c = Command::new("kitty");
            c.args(["--class", nvim_class]).arg("-e").arg("nvim");
            c
        }
    };

    // Guarded, because `current_dir` on a missing directory makes `spawn` fail
    // outright and Mode A deliberately warns-and-continues in that case.
    if project.is_dir() {
    ...
    command
        .spawn()
        .with_context(|| format!("Failed to launch {}", editor.name()))?;
    Ok(())
}

/// Spawn a layout's two terminals.
///
/// Dwindle maps the first into the top-right pane, so for Neovim layouts that
/// one shows the generated keybind reference — the same shape workspace 1's
/// dashboard uses. The second is a free shell, which is where `claude` and
/// `codex` get run when you would rather have them outside the editor. Zed
/// layouts get two plain shells: Zed has its own command palette.
fn spawn_layout_terminals(editor: Editor, term_class: &str, project: &Path) -> Result<()> {
    for index in 0..2 {
        let mut command = Command::new("kitty");
        command.args(["--class", term_class]);

        if editor == Editor::Neovim && index == 0 {
            if let Some(keybinds) = nvim_keybinds_path() {
                command.arg("-e").arg("less").arg(keybinds);
            }
        }
...
/// Absolute path of the generated Neovim keybind reference.
///
/// A missing file is deliberately not an error: the pane falls back to a plain
/// shell, which beats refusing to build the layout at all.
fn nvim_keybinds_path() -> Option<PathBuf> {
    let home = env::var_os("HOME")?;
    let path = PathBuf::from(home).join(NVIM_KEYBINDS_REL);
    path.is_file().then_some(path)
}

/// Whether a workspace currently holds any windows.
///
/// This is what makes the fixed-workspace layouts idempotent. It reuses the
/// same conservative reading `first_free_pool_workspace` applies to the pool:
/// anything we cannot positively prove is empty counts as occupied, so the
/// failure mode is "focus what is there" rather than "stack another layout".
fn workspace_is_occupied(workspace: u32) -> Result<bool> {
    let json = hyprctl_stdout(&["clients", "-j"])?;
    Ok(clients_on_workspace(&json, workspace)? > 0)
}
...
fn new_dev_space(path: Option<&str>, editor: Editor) -> Result<()> {
    ...
    register_pool_rules(target, &project.name, editor)?;
    ...
    spawn_editor(editor, &project.path, &pool_nvim_class(target))?;
    thread::sleep(Duration::from_secs(1));

    let term_class = pool_term_class(target);
    spawn_layout_terminals(editor, &term_class, &project.path)?;
...
/// Class the Mode B Neovim editor window is launched with, and matched on.
///
/// Workspace-scoped for the same reason as `pool_term_class`: a leftover rule
/// for one pool workspace must not capture a window meant for another.
fn pool_nvim_class(workspace: u32) -> String {
    format!("devpool-nvim-{workspace}")
}
...
fn register_pool_rules(target: u32, project_name: &str, editor: Editor) -> Result<()> {
    ...
    // Zed has no `--class`, so its rule must pin class AND title, which is what
    // couples Mode B placement to the project folder name. Kitty does support
    // `--class`, so the Neovim rule matches class alone: a project whose folder
    // is named something unexpected still lands on the right workspace.
    let editor_match = match editor {
        Editor::Zed => {
            let class = lua_string(ZED_CLASS_PATTERN)?;
            let title = lua_string(&format!("^({})$", re2_escape(project_name)))?;
            format!("{{ class = {class}, title = {title} }}")
        }
        Editor::Neovim => {
            let class = lua_string(&format!("^({})$", re2_escape(&pool_nvim_class(target))))?;
            format!("{{ class = {class} }}")
        }
    };
